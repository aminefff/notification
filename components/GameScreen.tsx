      ...
      <main className="flex-1 flex flex-col items-center justify-center overflow-hidden">
          <div style={{ transform: `scale(${scale})`, transformOrigin: 'center center', width: '100%', maxWidth: '1024px' }} className="flex flex-col items-center justify-center p-4 space-y-8">
              ...
              <footer className="flex gap-6 mt-4">
                  <button onClick={() => { if(!lifelines.fiftyFifty || revealed) return; playLifelineSound(); setDisabledOptions([0,1,2,3].filter(i=>i!==question.correctAnswerIndex).sort(()=>0.5-Math.random()).slice(0,2)); setLifelines({...lifelines, fiftyFifty:false}); }} disabled={!lifelines.fiftyFifty || revealed} className={`w-16 h-16 rounded-full border-2 flex items-center justify-center font-black transition-all ${!lifelines.fiftyFifty ? 'opacity-20 grayscale' : 'bg-brand/5 border-brand text-brand hover:bg-brand hover:text-black'}`}><span className="text-xs">50:50</span></button>
                  <button onClick={() => { if(!lifelines.askAudience || revealed) return; playLifelineSound(); alert("الجمهور يميل لـ: "+['أ','ب','ج','د'][question.correctAnswerIndex]); setLifelines({...lifelines, askAudience:false}); }} disabled={!lifelines.askAudience || revealed} className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all ${!lifelines.askAudience ? 'opacity-20 grayscale' : 'bg-blue-600/5 border-blue-500 text-blue-500'}`}><Users size={24}/></button>
                  <button onClick={() => { if(!lifelines.callFriend || revealed) return; playLifelineSound(); alert("الأستاذ يرجح: " + question.options[question.correctAnswerIndex]); setLifelines({...lifelines, callFriend:false}); }} disabled={!lifelines.callFriend || revealed} className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-all ${!lifelines.callFriend ? 'opacity-20 grayscale' : 'bg-purple-600/5 border-purple-500 text-purple-500'}`}><BookOpen size={24}/></button>
              </footer>
          </div>
      </main>
